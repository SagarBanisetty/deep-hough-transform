...
# choose device (CPU if GPU_ID < 0 or CUDA not available)
device = torch.device(
    "cuda" if torch.cuda.is_available() and CONFIGS["TRAIN"]["GPU_ID"] is not None and CONFIGS["TRAIN"]["GPU_ID"] >= 0
    else "cpu"
)
...
def main():
    ...
    if CONFIGS["TRAIN"]["DATA_PARALLEL"] and device.type == "cuda":
        logger.info("Model Data Parallel on CUDA")
        model = nn.DataParallel(model).to(device)
    else:
        if CONFIGS["TRAIN"]["DATA_PARALLEL"] and device.type != "cuda":
            logger.info("DATA_PARALLEL=True but CUDA not available; using single-device (CPU) model.")
    ...
    if args.resume:
        if isfile(args.resume):
            logger.info("=> loading checkpoint '{}'".format(args.resume))
            # map_location ensures loading works on CPU as well
    ...
    for epoch in range(start_epoch, CONFIGS["TRAIN"]["EPOCHS"]):
        ...
        logger.info(
            ...
            .format(
                epoch, CONFIGS["TRAIN"]["EPOCHS"], CONFIGS["MISC"]["TMP"],
